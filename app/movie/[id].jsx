...
// ── Static content ─────────────────────────────────────────────────────────────
const MovieStaticContent = memo(({ movie, summary, inLibrary, onToggleLibrary, watchProviders, onPersonPress }) => (
    <View>
        ...
        <View style={styles.infoText}>
          ...
          {movie.releaseDate && (
              <View style={styles.metaRow}>
                ...
                <Text style={styles.meta}> {movie.releaseDate.substring(0, 4)}</Text>
              </View>
          )}
          {movie.tmdbRating && (
              <View style={styles.metaRow}>
...
export default function MovieDetail() {
  ...
  const loadMovie = async () => {
    try {
      ...
      const [libraryStatus, providersRes] = await Promise.all([
        ...
        getMovieWatchProviders(data.tmdbId),
      ]);
  ...
  const loadMoreReviews = async () => {
    ...
    try {
      ...
      const { data } = await getReviews(movie.tmdbId, movie.mediaType, nextPage);
